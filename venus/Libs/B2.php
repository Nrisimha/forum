<?php
namespace Shared;
//https://github.com/Savjee/php-backblaze-b2-uploader
class B2{
	...
	public function uploadFile($path, $filename){
		...
		$headers = array();
		$headers[] = "Authorization: " . $this->getAuthorizationToken();
		$headers[] = "X-Bz-File-Name: " . $filename;//$folder . basename($path);
	...
	private function curlRequest($httpMethod = 'GET', $url, array $headers = [], $postFields = [], $uploadFile = false){
		...
		curl_setopt($session, CURLOPT_SSL_VERIFYPEER, false);//@todo remove in production
		...
		if(curl_getinfo($session, CURLINFO_HTTP_CODE) != 200){
			throw new \Exception("Didn't get a 200 response code: " . $server_output);
		}
		// Bye bye CURL!
		curl_close ($session);
